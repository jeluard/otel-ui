...
/** Returns unique non-empty instance_ids found across all spans of a trace. */
export function traceInstanceIds(spans: SpanEvent[]): string[] {
  const seen = new Set<string>();
  for (const s of spans) { if (s.instance_id) seen.add(s.instance_id); }
  return [...seen];
}
...
const TracesPanel = forwardRef<TracesPanelHandle, TracesPanelProps>(
  function TracesPanel({ activeTab, inFlightSpans, getEdges, getLayoutNodes, onTraceHighlightChange }, ref) {
    ...
    const { hiddenInstances } = useHideRules();
    const instVisible = (spans: SpanEvent[]) =>
      hiddenInstances.size === 0 || traceInstanceIds(spans).some(iid => !hiddenInstances.has(iid));
        ...
        <div id="trc-list-panel">
              ...
              <div id="trc-pinned">
                ...
                <div className="trc-item trc-selected" title={pinned.instance_id || undefined}>
                  ...
                  <div className="trc-item-meta">
                    ...
                    {traceInstanceIds(pinned.spans).map(iid => (
                      <span
                        key={iid}
                        className="trc-inst-dot"
                        title={iid}
                        style={{ background: instanceColor(iid).fill }}
                      />
                    ))}
                  </div>
                </div>
          ...
          >
            {(listFrozen ? frozenTracesRef.current : traces)
              .filter(t => t.trace_id !== selectedId && instVisible(t.spans))
              .map(trace => {
              ...
              return (
                ...
                >
                  ...
                  <div className="trc-item-meta">
                    ...
                    <span className="trc-item-cnt">{spanCount} span{spanCount !== 1 ? 's' : ''}</span>
                    {traceInstanceIds(trace.spans).map(iid => (
                      <span
                        key={iid}
                        className="trc-inst-dot"
                        title={iid}
