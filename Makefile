...
.PHONY: docker-run
docker-run: ## Run the bridge image locally  (-p 4317 OTLP gRPC, -p 4318 OTLP HTTP, -p 8080 WS)
        docker run --rm -p 4317:4317 -p 4318:4318 -p 8080:8080 ghcr.io/$(GHCR_USER)/otel-ui-bridge:$(BRIDGE_TAG)
.PHONY: docker-push
...
.PHONY: drop-db
drop-db: ## Delete the SQLite database (DB_PATH=./otel-ui.db)
	rm -f $(DB_PATH) $(DB_PATH)-wal $(DB_PATH)-shm
	@echo "  Dropped $(DB_PATH)"
